#ifndef _ROS_livox_ros_driver_CustomMsg_h
#define _ROS_livox_ros_driver_CustomMsg_h
...
namespace livox_ros_driver
{
  ...
  class CustomMsg : public ros::Msg
  {
    public:
      ...
      typedef livox_ros_driver::CustomPoint _points_type;
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      if(points_lengthT > points_length)
        this->points = (livox_ros_driver::CustomPoint*)realloc(this->points, points_lengthT * sizeof(livox_ros_driver::CustomPoint));
      points_length = points_lengthT;
      for( uint32_t i = 0; i < points_length; i++){
      offset += this->st_points.deserialize(inbuffer + offset);
        memcpy( &(this->points[i]), &(this->st_points), sizeof(livox_ros_driver::CustomPoint));
      }
     return offset;
    }

    const char * getType(){ return "livox_ros_driver/CustomMsg"; };
    const char * getMD5(){ return "e4d6829bdfe657cb6c21a746c86b21a6"; };
